...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    #    prefix += "/OMI/processed"
        ...
        prefix += "/EEG/processed"
    elif dataset == "EEG2":
        prefix += "/EEG2/processed/chb055"
    else:
...
class ConditionalDatasetLoader(Dataset):
    def __init__(
        self, 
        data=None, 
        window=None, 
        ...
        # 配置检查函数
        def check_config_changed(old_config, new_config):
            """
            比较配置是否发生变化
        ...
        if cache_path and os.path.exists(cache_path):
            try:
                # 尝试加载旧配置
                old_checkpoint = torch.load(cache_path)
                old_config = old_checkpoint.get('config', {})
                
                # 比较配置
                regenerate = check_config_changed(old_config, default_config)
        ...
        if not regenerate:
            ...
            # 生成新数据并保存
            assert all([data is not None, window is not None, prompt_generator is not None]), \
                "生成数据需要提供data、window和prompt_generator"
            ...
            # 生成数据
            self.data_windows, self.data_y, self.prompts = self.generate_data()
            
            # 保存数据和配置
            if cache_path:
                self.save_to_cache(cache_path, default_config)
    ...
    def generate_data(self):
        ...
        Returns:
            tuple: 包含窗口数据、目标数据和prompt的元组
        """
        window_list, y_list, prompt_list = [], [], []
        seq_len, feature = self.data.shape
        
        for index in range(0, seq_len - self.window - self.horizon + 1, self.stride):
            x = self.data[index:index + self.window, :]
            y = self.data[index + self.window: index + self.window + self.horizon, :]
            prompt = self.prompt_generator.generate_prompt(x)
            
            window_list.append(x)
            y_list.append(y)
            prompt_list.append(prompt)
        
        data_windows = torch.stack(window_list)
        data_y = torch.stack(y_list)
        # prompts = torch.stack(prompt_list)
        prompts = prompt_list
        
        return data_windows, data_y, prompts
    
    def save_to_cache(self, cache_path, config):
        """
        保存数据到缓存
    ...
    def load_from_cache(self, cache_path):
        ...
        self.data_windows = checkpoint['data_windows']
        self.data_y = checkpoint['data_y']
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: 包含窗口数据、目标数据和prompt的元组
        """
        x_with_prompt = self.data_windows[index]
        y = self.data_y[index]
        prompt = self.prompts[index]
        return x_with_prompt, y, prompt
