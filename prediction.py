...
class Predictor:
    ...
    def __init__(self, model, window_size, n_features, pred_args, summary_file_name="summary.txt"):
        ...
        self.batch_size = pred_args["batch_size"]
        self.model_name = pred_args["model"]
        self.use_cuda = True
    ...
    def get_score(self, values):
        ...
        with torch.no_grad():
            for x, y in tqdm(loader):
                ...
                if self.model_name == "llm" or self.model_name == "ofa" or self.model_name == "causalllm":
                   _, _, y_hat = self.model(x)
                else:
                    y_hat = self.model(x)

                y_hat = y_hat if y_hat.ndim > 1 else y_hat.unsqueeze(0)

                preds.append(y_hat.float().detach().cpu().numpy()) # 拼接预测值.
        ...
        actual = values.float().detach().cpu().numpy()[self.window_size:]  # value shape:(train set shape -win, features)
        ...
        anomaly_scores_topk = self.get_f1_scores(anomaly_scores, topk=15)
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        if true_anomalies is not None:
            bf_eval = bf_search(test_anomaly_scores_max_s, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)
            bf_eval2 = bf_search(test_anomaly_scores_mean_s, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)
            bf_eval3 = bf_search(test_anomaly_scores_topk_s, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)
            
            # bf_eval_nomov = bf_search(test_anomaly_scores_max, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)
            # bf_eval2_nomov = bf_search(test_anomaly_scores_mean, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)
            # bf_eval3_nomov = bf_search(test_anomaly_scores_topk, true_anomalies, k=100, start=0, end=80, step_num=1000, verbose=False)

            bf_eval_adjust = bf_search(test_anomaly_scores_max_s, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)
            bf_eval_adjust_topk = bf_search(test_anomaly_scores_topk_s, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)
            bf_eval_adjust_mean = bf_search(test_anomaly_scores_mean_s, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)

            # bf_eval_adjust_nomov = bf_search(test_anomaly_scores_max, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)
            # bf_eval_adjust_topk_nomov = bf_search(test_anomaly_scores_topk, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)
            # bf_eval_adjust_mean_nomov = bf_search(test_anomaly_scores_mean, true_anomalies, self.k, start=0, end=80, step_num=1000, verbose=False)
        else:
            bf_eval = {}
            bf_eval2 = {}
            bf_eval3 = {}
        ...
        # print(f"Results using best f1 score search nomov (MAX):\n {bf_eval_nomov}")
        # print(f"Results using best f1 score search nomov (MEAN):\n {bf_eval2_nomov}")
        # print(f"Results using best f1 score search nomov (TOPK):\n {bf_eval3_nomov}")
        ...
        # print(f"Results using best f1 score search nomov (Max_adjust):\n {bf_eval_adjust_nomov}")
        # print(f"Results using best f1 score search nomov (Topk_adjust):\n {bf_eval_adjust_topk_nomov}")
        # print(f"Results using best f1 score search nomov (Mean_adjust):\n {bf_eval_adjust_mean_nomov}")
        ...
        # for k, v in bf_eval_nomov.items():
        #     bf_eval_nomov[k] = float(v)
        # for k, v in bf_eval2_nomov.items():
        #     bf_eval2_nomov[k] = float(v)
        # for k, v in bf_eval3_nomov.items():
        #     bf_eval3_nomov[k] = float(v)
        ...
        # for k, v in bf_eval_adjust_nomov.items():
        #     bf_eval_adjust_nomov[k] = float(v)
        # for k, v in bf_eval_adjust_topk_nomov.items():
        #     bf_eval_adjust_topk_nomov[k] = float(v)
        # for k, v in bf_eval_adjust_mean_nomov.items():
        #     bf_eval_adjust_mean_nomov[k] = float(v)
        ...
        summary = {"epsilon_result": e_eval, "pot_result": p_eval, "bf_result_max": bf_eval, "bf_result_mean": bf_eval2, "bf_result_topk": bf_eval3, "bf_result_max_adjust": bf_eval_adjust,
                ...
                #    ,"bf_result_nomov_max": bf_eval_nomov, "bf_result_nomov_mean": bf_eval2_nomov, 
                #    "bf_result_nomov_topk": bf_eval3_nomov, "bf_result_nomov_max_adjust": bf_eval_adjust_nomov, 
                #    "bf_result_nomov_adjust_topk": bf_eval_adjust_topk_nomov, 
                #    "bf_result_nomov_max_adjust_mean": bf_eval_adjust_mean_nomov}
