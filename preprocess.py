...
def load_and_save(category, filename, dataset, dataset_folder, output_folder):
    ...
    with open(path.join(output_folder, dataset + "_" + category + ".pkl"), "wb") as file1:
        dump(temp, file1)
    temp = pd.DataFrame(temp)
    temp.to_csv(path.join(output_folder, dataset + "_" + category + ".csv"), index=None)
...
def load_data(dataset):
    ...
    if dataset == "SMD":
        ...
        labels.to_csv(path.join(output_folder, dataset + "_" + "test_label" + ".csv"), index=None)
        ...
        def concatenate_and_save(category):
            ...
            data.to_csv(path.join(output_folder, dataset + "_" + category + ".csv"), index=None)
        ...
        with open(path.join(output_folder, dataset + "_" + "train" + ".pkl"), "wb") as file:
            dump(X_train, file)
        with open(path.join(output_folder, dataset + "_" + "train" + ".csv"), "wb") as file:
            dump(X_train, file)

        X_test = test_df_.values
        with open(path.join(output_folder, dataset + "_" + "test" + ".pkl"), "wb") as file:
            dump(X_test, file)
        with open(path.join(output_folder, dataset + "_" + "test" + ".csv"), "wb") as file:
            dump(X_test, file)
        ...
        with open(path.join(output_folder, dataset + "_" + 'test_label' + ".pkl"), "wb") as file:
            dump(y_test, file)
        with open(path.join(output_folder, dataset + "_" + 'test_label' + ".csv"), "wb") as file:
            dump(y_test, file)
        ...
        makedirs(output_folder, exist_ok=True)
        train_df = pd.read_csv(dataset_folder+"train_orig_03.csv", delimiter=",")
        ...
        test_df = pd.read_csv(dataset_folder + "test_orig_03.csv", delimiter=",")
