    ...
    def fit(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            for x, y in train_loader:
                ...
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    latent_loss, kl_loss, preds = self.model(x)
                else:
                    preds = self.model(x)   
                ...
                if y.ndim == 3:
                    y = y.squeeze(1)
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    L1loss = self.forecast_criterion(y, preds)
                    loss = L1loss + latent_loss/(latent_loss/L1loss+1e-8).detach() + kl_loss/(kl_loss/L1loss+1e-8).detach()
                else:
                ...
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    forecast_mse_losses.append(L1loss.item())
            ...
            if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                forecast_mse_losses = np.array(forecast_mse_losses)
            forecast_epoch_loss = forecast_b_losses.mean()
            if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                forecast_mse_loss = forecast_mse_losses.mean()

            self.losses["train_forecast"].append(forecast_epoch_loss)
            if self.args.model == "llm"  or self.args.model == "ofa" or self.args.model == "causalllm":
                self.losses["train_mse"].append(forecast_mse_loss)
            # Evaluate on validation set
            forecast_val_loss = "NA"
            if val_loader is not None:
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    forecast_val_loss, val_forecast_mse_loss = self.evaluate(val_loader)
                else:
                    forecast_val_loss = self.evaluate(val_loader)
                self.losses["val_forecast"].append(forecast_val_loss)
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    self.losses["val_mse"].append(val_forecast_mse_loss)
            ...
            if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                if epoch % self.print_every == 0:
                    s = (
                        ...
                        f"mse_loss = {forecast_mse_loss:.5f}, "
                    )

                    if val_loader is not None:
                        s += (
                            f" ---- val_forecast_loss = {forecast_val_loss:.5f}, "
                            f"val_mse_loss = {val_forecast_mse_loss:.5f}"
                        )
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for x, y in data_loader:
                ...
                if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
                    L1loss = self.forecast_criterion(y, preds)
                    #forecast_loss = L1loss + latent_loss + kl_loss
                    forecast_loss = L1loss + latent_loss/(latent_loss/L1loss+1e-8).detach() + kl_loss/(kl_loss/L1loss+1e-8).detach()
                else:
                    forecast_loss = self.forecast_criterion(y, preds)
                forecast_losses.append(forecast_loss.item())
                if self.args.model == "llm" or self.args.model == "ofa":
                    val_forecast_mse_losses.append(L1loss.item())

        forecast_losses = np.array(forecast_losses)
        forecast_loss = (forecast_losses + 1e-8).mean()
        if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
            val_forecast_mse_losses = np.array(val_forecast_mse_losses)
            val_forecast_mse_loss = (val_forecast_mse_losses + 1e-8).mean()
        if self.args.model == "llm" or self.args.model == "ofa" or self.args.model == "causalllm":
            return forecast_loss, val_forecast_mse_loss
        else:
