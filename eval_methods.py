...
# def pak(scores, targets, thres, k, window_size=200):
#     """
#     :param scores: anomaly scores
#     :param targets: target labels
#     :param thres: anomaly threshold
#     :param k: percentage threshold for anomaly detection
...
#     n_windows = len(scores) // window_size

#     # Adjust predictions within each window
#     for i in range(n_windows):
#         start_idx = i * window_size
#         end_idx = start_idx + window_size

#         # Count the number of detected anomalies in the current window
#         anomalies_in_window = predicts[start_idx:end_idx].sum()

#         # Check if anomalies exceed the threshold
#         if anomalies_in_window > k / 100 * window_size:
#             predicts[start_idx:end_idx] = 1

#     # Handle the remaining points if data length is not divisible by window_size
#     remaining_start = n_windows * window_size
#     if remaining_start < len(scores):
#         remaining_anomalies = predicts[remaining_start:].sum()
#         if remaining_anomalies > k / 100 * (len(scores) - remaining_start):
#             predicts[remaining_start:] = 1
...
def pak(scores, targets, thres, k, window_size=200):
    ...
    # Identify regions where target labels are 1
    one_start_idx = np.where(np.diff(actuals, prepend=0) == 1)[0]
    zero_start_idx = np.where(np.diff(actuals, prepend=0) == -1)[0]

    # Ensure indices are correctly paired
    if len(one_start_idx) == len(zero_start_idx) + 1:
        zero_start_idx = np.append(zero_start_idx, len(scores))

    for start, end in zip(one_start_idx, zero_start_idx):
        # Adjust predictions within each '1' range using sliding window
        range_length = end - start
        n_windows = range_length // window_size
        ...
        for i in range(n_windows):
            ...
            # Count the number of detected anomalies in the current window
            anomalies_in_window = predicts[window_start:window_end].sum()

            # Check if anomalies exceed the threshold
            if anomalies_in_window > k / 100 * window_size:
                predicts[window_start:window_end] = 1

        # Handle remaining points in the range
        remaining_start = start + n_windows * window_size
        if remaining_start < end:
            remaining_anomalies = predicts[remaining_start:end].sum()
            if remaining_anomalies > k / 100 * (end - remaining_start):
                predicts[remaining_start:end] = 1
...
def calc_point2point(predict, actual):
    ...
    return float(f1), float(precision), float(recall), int(TP), int(TN), int(FP), int(FN), float(roc_auc)
